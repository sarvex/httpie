...
class TestHTTPie:
    ...
    def test_DELETE(self, httpbin):
        r = http('DELETE', f'{httpbin.url}/delete')
        assert HTTP_OK in r

    def test_PUT(self, httpbin):
        r = http('PUT', f'{httpbin.url}/put', 'foo=bar')
        assert HTTP_OK in r
        assert r.json['json']['foo'] == 'bar'

    def test_POST_JSON_data(self, httpbin):
        r = http('POST', f'{httpbin.url}/post', 'foo=bar')
        assert HTTP_OK in r
        assert r.json['json']['foo'] == 'bar'

    def test_POST_form(self, httpbin):
        r = http('--form', 'POST', f'{httpbin.url}/post', 'foo=bar')
        assert HTTP_OK in r
        assert '"foo": "bar"' in r

    def test_POST_form_multiple_values(self, httpbin):
        r = http('--form', 'POST', f'{httpbin.url}/post', 'foo=bar', 'foo=baz')
        assert HTTP_OK in r
    ...
    def test_POST_stdin(self, httpbin):
        with open(FILE_PATH) as f:
            ...
            r = http('--form', 'POST', f'{httpbin.url}/post', env=env)
        assert HTTP_OK in r
        assert FILE_CONTENT in r

    def test_headers(self, httpbin):
        r = http('GET', f'{httpbin.url}/headers', 'Foo:bar')
        assert HTTP_OK in r
